...
static const char dec[] = { 0,1,2,3,4,5,6,7,8 };
...
// STRUCTURES
typedef struct Stub_LED {
	uint8_t broj_stubaca;
	uint8_t broj_LED_dioda;
}stub_LED;
...
void main_demo(void) {
	...
	// QUEUES
	LEDBar_Queue = xQueueCreate(3, sizeof(uint32_t));
...
void LEDBar_Task(void* pvParameters) {
	...
	while (1) {
		xQueueReceive(LEDBar_Queue, &broj_queue, portMAX_DELAY);
		num = broj_queue.broj_LED_dioda;
		printf("ispis: %u\n", num);
		res = pow(2, num) - 1;
		printf("primio karakter: %u\n", broj_queue.broj_stubaca);// prikazuje primljeni karakter u cmd prompt
		printf("primio karakter: %u\n", res);// prikazuje primljeni karakter u cmd prompt
		set_LED_BAR(broj_queue.broj_stubaca, res);
	}
}
...
void SerialReceive_Task(void* pvParameters) {
	...
	while (1) {
		...
		if (cc == 0xef) {	// EF oznacava POCETAK poruke
			...
			broj.broj_stubaca = r_buffer[0];
			broj.broj_LED_dioda = r_buffer[1];
			// 	printf("primio karakter: %u\n", broj.broj_stubaca);// prikazuje primljeni karakter u cmd prompt
		// 	printf("primio karakter: %u\n", broj.broj_LED_dioda);// prikazuje primljeni karakter u cmd prompt
			xQueueSend(LEDBar_Queue, &broj, portMAX_DELAY);
		}
